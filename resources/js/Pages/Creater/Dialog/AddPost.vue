<template>
    <el-dialog v-model="dialogVisible" id="add-post" class="bg-[#f4f1f8] max-h-[90%] overflow-scroll" width="1000px" 
     style="margin-top: 18px !important;" :show-close="true">
        ...
        <div class="mx-[4px] mt-[42px]">
            <div class="mt-[18px]">
                <div class="text-[16px] font-bold text-[#000]">Tiêu đề bài viết <span class="text-[14px]">(bắt buộc)</span></div>
            ...
            <div class="mt-[18px]">
                <div class="text-[16px] font-bold text-[#000]">Mô tả bài viết</div>
            ...
            <div class="mt-[18px]">
                <div class="text-[16px] font-bold text-[#000]">Nội dung bài viết<span class="text-[14px]">(bắt buộc)</span></div>
        ...
        <template #footer>
            <div class="pb-[12px] pt-[8px]">
                <div class="flex justify-center text-[14px]">
                    <div class="cursor-pointer flex justify-center items-center w-[180px] mr-[18px] rounded-[4px] bg-[#9f224e] py-[4px] h-[32px] text-[15px] text-white"
                        @click="addPost">
                        Phân loại bài viết
                    </div>
                    <div @click="cancel"
        ...
        <ShowPostForm ref="showCategoryPost" @change-category="createPost"/>
    </el-dialog>
</template>
...
export default {
    ...
    methods: {
        ...
        addPost() {
            ...
            axios.post(route('creator.posts.validate', this.id), pagram)
                .then(() => {
        ...
        createPost(category) {
            ...
            pagram.append('content', this.$refs['content'].editorData ?? '')
            pagram.append('category', this.categorySelect)
            axios.post(route('creator.posts.store'), pagram)
            .then(() => {
                    ElMessage({
                        showClose: true,
